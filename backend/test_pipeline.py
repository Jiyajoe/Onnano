"""
test_pipeline.py - Test script to verify the complete CV, normalization, slicing, and comparison pipeline.
"""
...
from app.cv.features import extract_visual_features
...
def run_tests():
    ...
    res = process_single_object_pipeline(pencil1, parts_count=4)
    ...
    print("Texture:", res["object"]["texture"]["descriptor"])
    ...
    for i, img in enumerate(objs):
        ...
        col = extract_color_analysis(ori.normalized_bgr, ori.normalized_mask)
        tex = extract_texture_analysis(ori.normalized_bgr, ori.normalized_mask)
        edg = extract_edge_analysis(ori.normalized_bgr, ori.normalized_mask)
        feat = extract_visual_features(ori.normalized_bgr, ori.normalized_mask)
        ...
        data_list.append({
            ...
            "mask": ori.normalized_mask, "bgr": ori.normalized_bgr,
            "ident": ident,
        })
        print(f"Object {i+1}: {ident.detected_type} ({ident.confidence_pct}%) - {col.dominant_name}")
    ...
    for i in range(len(data_list)):
        for j in range(i+1, len(data_list)):
            ...
            print(f"  Texture: {comp.texture_similarity}% | Features: {comp.feature_similarity}% | Edges: {comp.edge_similarity}%")
            print(f"  --> OVERALL: {comp.overall_similarity}%")
    ...
    verdict = classify_relationship(
        ...
        same_type=(data_list[0]["ident"].detected_type == data_list[1]["ident"].detected_type),
        type1=data_list[0]["ident"].detected_type,
        type2=data_list[1]["ident"].detected_type,
    ...
    print("\n==================================================")
    print("ALL BACKEND CV & AI PIPELINE TESTS PASSED!")
    print("==================================================")
